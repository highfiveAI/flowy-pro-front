...
const NewMeetingPopup: React.FC<NewMeetingPopupProps> = ({
    ...
    const [attendees, setAttendees] = useState<Attendee[]>([
        (() => {
            ...
            return me ? { ...me } : { user_id: userId, name: '', email: '', user_jobname: '' };
        })()
    ]);
    ...
    const handleSelectAll = () => {
        if (allSelected) {
            ...
            setAttendees([
                attendees[0],
                ...attendees.slice(1).map(() => ({ user_id: '', name: '', email: '', user_jobname: '' }))
            ]);
    ...
    React.useEffect(() => {
        const restUsers = projectUsers.filter(u => u.user_id !== userId);
        const selectedIds = attendees.slice(1).map(a => a.user_id).filter(Boolean);
        setAllSelected(selectedIds.length === restUsers.length && restUsers.length > 0 && selectedIds.every(id => restUsers.some(u => u.user_id === id)));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 실제 API 호출
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/projects/meeting/create`, {
                method: "POST",
                headers: {
                    ...
                    <FormGroup>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
                            <input
                                ...
                                style={{ marginRight: 8 }}
                            />
                            <label htmlFor="selectAllAttendees" style={{ cursor: 'pointer', userSelect: 'none' }}>
                                전체 선택
                            </label>
                        </div>
                        {attendees.map((a, idx) => (
                            <AttendeeRow key={idx}>
                                {idx === 0 ? (
                                    <StyledSelect value={a.user_id} disabled style={{ flex: 1, marginRight: 8, color: '#888', background: '#f5f5f5' }}>
                                    ...
                                    >
                                        ...
                                        {projectUsers.filter(u => u.user_id !== userId && !attendees.some((att, i) => att.user_id === u.user_id && i !== idx)).map(u => (
                                ...
                                />
                                {idx === 0 ? (
                                    <span style={{ color: '#888', fontSize: 13, marginLeft: 8 }}>본인</span>
                                ) : (
