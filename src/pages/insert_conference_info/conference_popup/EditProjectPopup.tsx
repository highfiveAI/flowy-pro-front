...
const EditProjectPopup: React.FC<PopupProps> = ({ onClose, projectToEdit }) => {
  ...
  const [selectedProjectUsers, setSelectedProjectUsers] = useState<ProjectUserIdName[]>([]);
  ...
  const handleSelectUser = (user: ProjectUserIdName) => {
    if (!selectedProjectUsers.some((u) => u.user_id === user.user_id)) {
      setSelectedProjectUsers([...selectedProjectUsers, { ...user, role_id: ppId }]);
      setSearchTerm('');
    }
  };

  const handleDeselectUser = (user: ProjectUserIdName) => {
    setSelectedProjectUsers(selectedProjectUsers.filter((u) => u.user_id !== user.user_id));
  };
  ...
  const handleUpdateProject = async () => {
    ...
    if (!selectedProjectUsers.some((user) => user.role_id === poId)) {
      setErrorMessage('프로젝트에 PO 역할을 가진 참여자가 1명 이상 있어야 합니다.');
      return;
    }
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      // 3. 기존 참여자 정보 로드 및 설정
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/stt/project-users/${projectToEdit.projectId}`, {
          credentials: 'include',
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        const projectUsersData = await res.json();
        if (projectUsersData && projectUsersData.users) {
          const currentParticipants = projectUsersData.users.map((u: { user_id: string; name: string; user_jobname: string }) => {
            const role = allRoles.find((r: any) => r.role_name === u.user_jobname);
            return {
              user_id: u.user_id,
              user_name: u.name,
  ...
  return (
    <PopupOverlay>
      <PopupContent>
        ...
        <FormGroup>
          <StyledLabel htmlFor="project-attendees">프로젝트 참여자 선택</StyledLabel>
          <UserManagementContainer>
            <UserPanel>
              <SearchContainer>
                <SearchInput
              ...
              <UserListBox>
                {filteredUsers
                  .filter(u => !selectedProjectUsers.some(su => su.user_id === u.user_id))
                  .map((user) => (
                    <UserItem key={user.user_id}>
                      <span>{user.user_name}</span>
                      <AddButton onClick={() => handleSelectUser(user)}>+</AddButton>
                    </UserItem>
                  ))}
              </UserListBox>
            </UserPanel>
            <UserPanel>
              <SelectedUsersContainer>
                <SelectedUsersTitle>선택된 참여자 ({selectedProjectUsers.length}명)</SelectedUsersTitle>
                <TagsContainer>
                  {selectedProjectUsers.map((selectedUser) => (
                    <SelectedUserItem key={selectedUser.user_id}>
                      ...
                      {selectedUser.user_id !== user?.id && (
                         <RemoveButton onClick={() => handleDeselectUser(selectedUser)}>×</RemoveButton>
                      )}
                    </SelectedUserItem>
                  ))}
                </TagsContainer>
